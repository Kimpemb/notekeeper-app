...
import { importNotes, importNotesOverwrite, importNotesAsCopies } from "@/db/queries";
...
export function ImportModal() {
  ...
  const parseFile = useCallback(async (content: string, ext: string) => {
    try {
      ...
      if (ext === "md") {
        ...
        // Validate this looks like a NoteKeeper export
  ...
  // Tauri drag & drop via window events
  useEffect(() => {
    if (!importOpen) return;
            ...
            >
              ...
              <div className="text-center">
                <p className="text-sm text-zinc-600 dark:text-zinc-400">
                  Drop a <span className="font-medium">.json</span> or <span className="font-medium">.md</span> file here
                </p>
                ...
                <div className="max-h-36 overflow-y-auto">
                  {preview.notes.slice(0, 20).map((n) => {
                    ...
                    return (
                      <div
                        key={n.id}
                        className="flex items-center gap-2 px-3 py-2 border-b border-zinc-50 dark:border-zinc-800/50 last:border-0"
                      >
                        <span className="flex-1 text-sm text-zinc-600 dark:text-zinc-400 truncate">
                          {n.title}
                        </span>
                        {isDup && (
                          <span className="text-xs text-amber-500 shrink-0">duplicate</span>
                        )}
                      </div>
                    );
                  })}
                  {preview.notes.length > 20 && (
                    <div className="px-3 py-2 text-xs text-zinc-400 text-center">
                      +{preview.notes.length - 20} more notes
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-col items-center gap-3 py-8">
              <div className="w-10 h-10 rounded-full bg-green-100 dark:bg-green-950/40 flex items-center justify-center">
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                  <path d="M4 9l4 4 6-7" stroke="#22c55e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
              <div className="text-center">
                <p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                  {imported} {imported === 1 ? "note" : "notes"} imported
                </p>
          ...
          {stage === "error" && (
            <div className="flex flex-col items-center gap-3 py-6">
              <div className="w-10 h-10 rounded-full bg-red-100 dark:bg-red-950/40 flex items-center justify-center">
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                  <path d="M9 5v5M9 13h.01" stroke="#ef4444" strokeWidth="2" strokeLinecap="round"/>
              ...
              <button
                ...
                className="text-sm text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors duration-150"
              >
                Try again
              </button>
            </div>
