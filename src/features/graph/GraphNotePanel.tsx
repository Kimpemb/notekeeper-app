...
//
// Unified right-side panel for the graph view.
// Four modes — state machine priority: edge > edit > detail > preview > idle
//
//   preview  — hover, shows headings/snippet/tags/links
//   detail   — single click locks it, adds word count + backlinks + Edit button
//   edit     — embeds GraphNodeEditor at 40% of window width, graph auto-fullscreens
//   edge     — edge click, shows backlink context + delete confirm
//
// Edit mode automatically triggers fullscreen graph (handled in GraphView).
// The panel floats over the graph — no canvas compression, no simulation restart.
...
const PANEL_WIDTH_DEFAULT = 256;
// Edit mode: 40% of current window width, min 360px
const getEditPanelWidth = () => Math.max(360, Math.round(window.innerWidth * 0.4));
...
export function GraphNotePanel({
  ...
  useEffect(() => {
    if (mode === "edit") {
      setEditPanelWidth(getEditPanelWidth());
    }
  }, [mode]);

  const panelWidth = mode === "edit" ? editPanelWidth : PANEL_WIDTH_DEFAULT;
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setPreview({
          headings:  extractHeadings(fetched?.content),
          snippet:   extractSnippet(fetched?.plaintext),
          tags:      node.tags,
          linkCount: node.linkCount,
          wordCount: countWords(fetched?.plaintext),
  ...
  }, [activeNodeId, mode]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => { if (edgeContext) setVisible(true); }, [edgeContext]);

  // ── Keyboard shortcut: E to enter edit mode from detail ───────────────────
  useEffect(() => {
            ...
            <div style={{ padding: "10px 14px 0", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                <button onClick={() => onOpen(edgeContext.sourceId)} style={{ background: "transparent", border: "none", cursor: "pointer", padding: 0, textAlign: "left" }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: LABEL, opacity: 0.9 }}>{edgeContext.sourceTitle}</span>
                </button>
                ...
                <button onClick={() => onOpen(edgeContext.targetId)} style={{ background: "transparent", border: "none", cursor: "pointer", padding: 0, textAlign: "left" }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: LABEL, opacity: 0.9 }}>{edgeContext.targetTitle}</span>
                </button>
              </div>
              {edgeContext.snippet ? (
                <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 6, padding: "8px 10px", marginBottom: 10 }}>
                  <p style={{ margin: 0, fontSize: 11, color: LABEL, opacity: 0.6, lineHeight: 1.65, fontStyle: "italic" }}>
                    "{edgeContext.snippet}"
                  </p>
                </div>
              ) : (
            ...
            <div style={{ padding: "0 14px 12px", flexShrink: 0 }}>
              {!confirmingDelete ? (
                <button
                  ...
                  style={{ width: "100%", padding: "7px 0", background: `${DANGER}18`, border: `1px solid ${DANGER}44`, borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 600, color: DANGER, letterSpacing: "0.02em", transition: "background 120ms" }}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <span style={{ fontSize: 11, color: LABEL, opacity: 0.55, textAlign: "center" }}>Remove this link?</span>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => setConfirmingDelete(false)} style={{ flex: 1, padding: "6px 0", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, cursor: "pointer", fontSize: 11, color: LABEL, opacity: 0.7 }}>Cancel</button>
                    <button
                      onClick={() => { setConfirmingDelete(false); onDeleteEdge(edgeContext.sourceId, edgeContext.targetId); }}
                      style={{ flex: 1, padding: "6px 0", background: DANGER, border: "none", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, color: "#fff" }}
                    >
                      Remove
                    </button>
                  </div>
          ...
          <>
            <div style={{
              ...
              <svg width="13" height="13" viewBox="0 0 13 13" fill="none" style={{ flexShrink: 0, opacity: 0.45 }}>
                <path d="M9 2l2 2-7 7H2v-2L9 2z" stroke={LABEL} strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
              ...
              <button
                ...
                style={{ background: "transparent", border: "none", cursor: "pointer", padding: 4, borderRadius: 4, opacity: 0.35, flexShrink: 0, display: "flex", alignItems: "center", transition: "opacity 120ms" }}
                onMouseEnter={e => (e.currentTarget.style.opacity = "0.9")}
                onMouseLeave={e => (e.currentTarget.style.opacity = "0.35")}
              >
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                  <path d="M5 2H2v8h8V7M7 2h3v3M10 2L5.5 6.5" stroke={LABEL} strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </button>
          ...
          <>
            <div style={{
              ...
              <button
                ...
                style={{ background: "transparent", border: "none", cursor: "pointer", padding: 0, textAlign: "left", flex: 1, display: "flex", alignItems: "center", gap: 6 }}
              >
                <span style={{ fontSize: 13, fontWeight: 600, color: LABEL, lineHeight: 1.35, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {activeNode?.title ?? ""}
                </span>
              ...
              {mode === "detail" && activeNode && (
                <>
                  <button
                    ...
                    style={{ background: `${ACCENT}18`, border: `1px solid ${ACCENT}44`, borderRadius: 5, cursor: "pointer", padding: "3px 8px", fontSize: 10, fontWeight: 600, color: ACCENT, flexShrink: 0, letterSpacing: "0.02em", transition: "background 120ms" }}
                  ...
                  <button
                    ...
                    style={{ background: "transparent", border: "none", cursor: "pointer", padding: 4, borderRadius: 4, opacity: 0.4, flexShrink: 0, display: "flex", alignItems: "center", transition: "opacity 120ms" }}
                    onMouseEnter={e => (e.currentTarget.style.opacity = "0.9")}
                    onMouseLeave={e => (e.currentTarget.style.opacity = "0.4")}
                  >
                    <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
            ...
            {mode === "detail" && preview && (
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "5px 12px", borderBottom: "1px solid rgba(255,255,255,0.05)", flexShrink: 0 }}>
                ...
                {preview.createdAt > 0 && (
                  <span style={{ fontSize: 11, color: LABEL, opacity: 0.35 }}>{formatDate(preview.createdAt)}</span>
                )}
              </div>
            )}
              ...
              {!loading && hasHeadings && (
                <ul style={{ listStyle: "none", margin: 0, padding: "5px 0" }}>
                  {preview!.headings.map((h, i) => (
                    <li key={i} style={{ opacity: animated ? 1 : 0, transform: animated ? "translateX(0)" : "translateX(-6px)", transition: `opacity 180ms ease ${i * 28}ms, transform 180ms ease ${i * 28}ms` }}>
                      <button
                        onClick={() => activeNode && onOpen(activeNode.id, h.text)}
                        style={{ width: "100%", display: "flex", alignItems: "center", gap: 6, paddingLeft: h.level === 1 ? 0 : 8 + (h.level - 1) * 10, paddingRight: 12, paddingTop: 5, paddingBottom: 5, background: "transparent", border: "none", borderLeft: h.level === 1 ? `2px solid ${ACCENT}` : "2px solid transparent", cursor: "pointer", textAlign: "left", transition: "background 80ms ease" }}
                        onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.05)")}
                        onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
                      >
                        <span style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", color: h.level === 1 ? ACCENT : LABEL, opacity: h.level === 1 ? 0.9 : 0.32, flexShrink: 0, letterSpacing: "0.04em", paddingLeft: h.level === 1 ? 10 : 0 }}>
                          H{h.level}
                        </span>
                        <span style={{ fontSize: h.level === 1 ? 12 : 11, fontWeight: h.level === 1 ? 600 : h.level === 2 ? 500 : 400, color: LABEL, opacity: h.level === 1 ? 0.88 : h.level === 2 ? 0.62 : 0.42, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>
                          {h.text}
                        </span>
              ...
              {mode === "detail" && (
                <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", marginTop: 4 }}>
                  ...
                  {!backlinksLoading && backlinks.length === 0 && (
                    <p style={{ margin: 0, padding: "4px 12px 10px", fontSize: 11, color: LABEL, opacity: 0.28, fontStyle: "italic" }}>No backlinks.</p>
                  )}
                  {!backlinksLoading && backlinks.map(bl => (
                    <button
                      key={bl.id}
                      onClick={() => onOpenBacklink(bl.id)}
                      style={{ width: "100%", display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", background: "transparent", border: "none", cursor: "pointer", textAlign: "left", transition: "background 80ms" }}
                      onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.05)")}
                      onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
                    >
            ...
            {!loading && preview && (
              <div style={{ padding: "5px 12px", borderTop: "1px solid rgba(255,255,255,0.05)", flexShrink: 0 }}>
                <span style={{ fontSize: 10, color: LABEL, opacity: 0.26 }}>
                  {mode === "detail"
                    ? "Press E to edit · Double-click to rename · Triple-click to open"
