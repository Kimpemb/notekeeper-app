...
export function NoteTreeItem({ noteId, depth }: Props) {
  ...
  function handleRename(e: React.MouseEvent) {
    e.stopPropagation(); setContextMenu(null);
    setRenameValue(note!.title); setRenaming(true);
  }
      ...
      >
        ...
        {renaming ? (
          ...
          <span className="flex-1 truncate text-base leading-none flex items-center gap-1.5">
            ...
            {isPinned && (
              <span className="shrink-0 opacity-40">
                <svg width="9" height="9" viewBox="0 0 10 10" fill="currentColor"><path d="M5 0L6.2 3.8H10L7 6.2L8.1 10L5 7.8L1.9 10L3 6.2L0 3.8H3.8L5 0Z"/></svg>
              </span>
            )}
          </span>
        )}
...
function ContextItem({ label, onClick, danger = false }: {
  label: string;
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left px-4 py-2.5 text-base transition-colors duration-75 ${danger ? "text-red-500 hover:bg-red-50 dark:hover:bg-red-950" : "hover:bg-zinc-100 dark:hover:bg-zinc-700"}`}
    >
      {label}
    </button>
  );
