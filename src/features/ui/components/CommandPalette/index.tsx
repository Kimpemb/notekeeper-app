...
type CommandItem = { id: string; label: string; description?: string; action: () => void; };
...
export function CommandPalette() {
  ...
  useEffect(() => {
    ...
    function handleMouseDown(e: MouseEvent) { if (panelRef.current && !panelRef.current.contains(e.target as Node)) closePalette(); }
  ...
  useEffect(() => {
    if (!paletteOpen) return;
    function handleKeyDown(e: KeyboardEvent) { if (e.key === "Escape") { e.preventDefault(); e.stopPropagation(); closePalette(); } }
  ...
  const staticCommands: CommandItem[] = [
    { id: "new-note", label: "New Note", description: "Create a blank note", action: async () => { await createNote(); closePalette(); } },
    { id: "toggle-theme", label: theme === "dark" ? "Switch to Light Mode" : "Switch to Dark Mode", description: "Toggle theme", action: () => { toggleTheme(); closePalette(); } },
  ];

  const noteCommands: CommandItem[] = notes.slice(0, 50).map((n) => ({
    id: `note-${n.id}`, label: n.title, description: n.plaintext.slice(0, 60) || "Empty note",
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh]">
      ...
      <div ref={panelRef} className="relative w-full max-w-lg mx-4 rounded-xl overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 shadow-2xl">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-100 dark:border-zinc-800">
          ...
          <input ref={inputRef} value={query} onChange={(e) => { setQuery(e.target.value); setSelected(0); }} onKeyDown={handleInputKeyDown}
          ...
          {filtered.map((item, i) => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                {item.description && <p className="text-sm text-zinc-400 dark:text-zinc-500 truncate mt-0.5">{item.description}</p>}
              </div>
              {i === selected && <kbd className="text-base text-zinc-400 bg-zinc-200 dark:bg-zinc-700 px-2 py-0.5 rounded font-mono shrink-0 leading-snug">↵</kbd>}
            </li>
          ))}
        </ul>
      </div>
