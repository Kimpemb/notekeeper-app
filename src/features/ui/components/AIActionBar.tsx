          ...
          <div className="space-y-3">
            <div className="flex flex-wrap gap-1.5">
              {result.tags.map((tag) => (
                <span
                  key={tag}
                  className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700"
                >
                  #{tag}
                </span>
              ))}
            ...
            <button
              onClick={() => result.tags && onApplyTags(result.tags)}
              disabled={applyingTags || tagsApplied}
            ...
            >
              {tagsApplied ? "✓ Tags applied" : applyingTags ? "Applying…" : "Apply tags to note"}
            </button>
          </div>
        ) : (
...
export function AIActionBar({ note }: AIActionBarProps) {
  ...
  async function handleApplyTags(tags: string[]) {
    ...
    try {
      // Merge with existing tags (comma-separated string in note.tags)
